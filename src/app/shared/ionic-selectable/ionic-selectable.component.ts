...
import {IonItem, IonNav, IonRouterOutlet, ModalController, Platform, PopoverController} from '@ionic/angular';
...
export class IonicSelectableComponent<T extends { id: string }> implements ControlValueAccessor, OnInit, DoCheck {

  @Input()
  modalOptions: ModalOptions = ({} as any);
  ...
  set value(value) {
    ...
    if (this.isMultiple) {
      ...
      if (!IonicSelectableComponent._isNullOrWhiteSpace(value)) {
  ...
  @Input() interface: 'popover' | 'modal' = 'modal';
  ...
  get _modalController() {
    return (this.interface === 'modal' ? this.$_modalController : this._popoverController) as ModalController;
  }
  ...
  constructor(
    ...
    @Optional() public graphqlPaginationService: GraphqlPaginationService
  ) {
  ...
  private _isOnSearchEnabled = false;
  ...
  onItemsChange: EventEmitter<any> = new EventEmitter();
  ...
  _itemsToConfirm: Array<T> = [];
  _selectedItems: Array<T> = [];
  _modalComponent: IonicSelectableModalComponent<T>;
  _filteredGroups: Array<{
    value: string;
    text: string;
    items: Array<T>;
  }> = [];
  ...
  @Input()
  addItemTemplateHasFooter = false;
   ...
   */
  @Input()
  virtualScrollApproxItemHeight = 56;
  @Input()
  virtualScrollEnableUnequalChildrenSizes = false;
  @Input()
  virtualScrollBufferAmount = 5;
   ...
   */
  @Input()
  searchFailText = 'Pas d\'elements a afficher';
   ...
   */
  @Output()
  onChange: EventEmitter<{ component: IonicSelectableComponent<T>, value: any }> = new EventEmitter();
   ...
   */
  @Output()
  onSearch: EventEmitter<{ component: IonicSelectableComponent<T>, text: string }> = new EventEmitter();
   ...
   */
  @Output()
  onSearchFail: EventEmitter<{ component: IonicSelectableComponent<T>, text: string }> = new EventEmitter();
   ...
   */
  @Output()
  onSearchSuccess: EventEmitter<{ component: IonicSelectableComponent<T>, text: string }> = new EventEmitter();
   ...
   */
  @Output()
  onInfiniteScroll: EventEmitter<{ component: IonicSelectableComponent<T>, text: string }> = new EventEmitter();
   ...
   */
  @Output()
  onOpen: EventEmitter<{ component: IonicSelectableComponent<T> }> = new EventEmitter();
   ...
   */
  @Output()
  onClose: EventEmitter<{ component: IonicSelectableComponent<T> }> = new EventEmitter();
   ...
   */
  @Output()
  onSelect: EventEmitter<{ component: IonicSelectableComponent<T>, item: any, isSelected: boolean }> = new EventEmitter();
   ...
   */
  @Output()
  onClear: EventEmitter<{ component: IonicSelectableComponent<T>, items: Array<any> }> = new EventEmitter();
   ...
   */
  @Output()
  onSaveItem: EventEmitter<{ component: IonicSelectableComponent<T> | any, item: T | any }> = new EventEmitter();
   ...
   */
  @Output()
  onDeleteItem: EventEmitter<{ component: IonicSelectableComponent<T> | any, item: T | any }> = new EventEmitter();
   ...
   */
  @Output()
  onAddItem: EventEmitter<{ component: IonicSelectableComponent<T> }> = new EventEmitter();
  ...
  addItemTemplate: TemplateRef<{ item: T }>;
  ...
  _setHasSearchText() {
    this._hasSearchText = !IonicSelectableComponent._isNullOrWhiteSpace(this._searchText);
    // ;
  }
  ...
  _formatItem(item: any): string {
    if (IonicSelectableComponent._isNullOrWhiteSpace(item)) {
      return null;
    }
  ...
  _filterItems() {
    ...
    if (this._hasOnSearch()) {
      ...
      if (!this._searchText || !this._searchText.trim()) {
        ...
        const filterText = TextService.normalize(this._searchText);
        let reg;
        try {
        ...
        this._groups.forEach(group => {
          const items: Array<any> = group.items.filter(item => {
      ...
      this._hasFilteredItems = !IonicSelectableComponent._areGroupsEmpty(groups);
  ...
  _deleteSelectedItem(item: any) {
    this._selectedItems = this._selectedItems.filter(
      (selectedItem, itemIndex) => this._getItemValue(item) !== this._getStoredItemValue(selectedItem));
  }
  ...
  _setItemsToConfirm(items: Array<any>) {
  ...
  _select(item: any) {
    ...
    if (this.isMultiple) {
      if (isItemSelected) {
        ...
        if (
          !(this.maxSelectable)
          || this._selectedItems.length < this.maxSelectable
        ) {
  ...
  private _getLabelText(): string {
    return this._ionLabelElement ? this._ionLabelElement.textContent : (this._label || null);
  }
  ...
  private _setItems(items: Array<any>) {
    ...
    if (items && items.length) {
      if (this._hasGroups) {
        ...
        items.forEach(item => {
          const groupValue = IonicSelectableComponent._getPropertyValue(item, this.groupValueField);
          ...
          if (group) {
            ...
            groups.push({
              value: groupValue,
              text: IonicSelectableComponent._getPropertyValue(item, this.groupTextField),
              items: [item]
            });
          }
        });
    ...
    this._hasFilteredItems = !IonicSelectableComponent._areGroupsEmpty(this._filteredGroups);
    // ;
    this.ref.markForCheck();
  ...
  private _setHasPlaceholder() {
    this._hasPlaceholder = !!(!this.hasValue() &&
      (!IonicSelectableComponent._isNullOrWhiteSpace(this.placeholder) || this.placeholderTemplate));
    // ;
  ...
  ngOnInit() {
    if (!this.routerOutlet) {
      this.routerOutlet = this.ionicDialogService.routerOutlet;
    }

    this._isIos = this._platform.is('ios');
    this._isMD = !this._isIos;
    this._hasObjects = !IonicSelectableComponent._isNullOrWhiteSpace(this.itemValueField);
  ...
  hasValue(): boolean {
    if (this.isMultiple) {
      ...
      return this._valueItems.length !== 0 && !IonicSelectableComponent._isNullOrWhiteSpace(this._valueItems[0]);
    }
  }
  ...
  open(): Promise<void> {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      const modalOptions: ModalOptions = {
        ...
        component: (self.interface === 'popover' ? IonicSelectableModalComponent : ModalBaseComponent),
        componentProps: {
          selectComponent: self,
          rootPage: IonicSelectableModalComponent,
        },
        backdropDismiss: self._shouldBackdropClose,
        ...(self.interface === 'popover' ? ({event: self.$event, showBackdrop: false} as any) : {})
   ...
   * @memberof IonicSelectableComponent
   */
  toggleItems(isSelect: boolean, items?: Array<any>) {
    if (isSelect) {
  ...
  private static _areGroupsEmpty(groups) {
    return groups.length === 0 || groups.every(group => {
      return !group.items || group.items.length === 0;
    });
  }

  private static _getPropertyValue(object: any, property: string): any {
    if (!property) {
      return null;
    }
